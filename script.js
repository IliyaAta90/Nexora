(() => {
  ...
  function initCustomCursor() {
    ...
    document.addEventListener("mouseover", (event) => {
      if (!event.target.closest("button, a, .feature-card")) return;
      ...
      for (let i = 0; i < particles.length; i++) {
        for (let j = i + 1; j < particles.length; j++) {
          ...
          if (distanceSquared < maxConnectionDistanceSquared) {
            ...
            context.globalAlpha =
              (1 - distance / maxConnectionDistance) * 0.08;
  ...
  function initAnimations() {
    ...
    if (document.querySelector("#features")) animateFeatures();
    if (document.querySelector("#logo-draw")) animateLogoDraw();
    if (document.querySelector("#process")) animateProcess();
    if (document.querySelector("#testimonial")) animateTestimonial();
    if (document.querySelector("#cta-final")) animateCta();
    if (document.querySelector("#hero")) animateHeroParallax();
  }
  ...
  function animateLogoDraw() {
    ...
    ScrollTrigger.create({
      ...
      onUpdate: ({ progress }) => {
        ...
        if (dom.logoPath) {
          dom.logoPath.style.strokeDashoffset = String(pathOffset);
        }

        if (progress > 0.85) {
          const revealOpacity = ((progress - 0.85) / 0.15).toString();

          if (dom.logoFill) dom.logoFill.style.opacity = revealOpacity;
          if (dom.logoGlow) dom.logoGlow.style.opacity = revealOpacity;
        }
      }
    });
  ...
  function initNavBehavior() {
    ...
    window.addEventListener(
      ...
      () => {
        ...
        dom.nav.style.transform =
          currentScrollY > lastScrollY && currentScrollY > 100
            ? "translateY(-100%)"
            : "translateY(0)";

        dom.nav.style.transition = "transform 0.4s ease";
  ...
  function applyLanguage() {
    ...
    if (dom.langBtn) {
      dom.langBtn.textContent = currentLang === "en" ? "فارسی" : "English";
    }
  }

  function initViewWork() {
    if (!dom.viewWorkBtn) return;

    dom.viewWorkBtn.addEventListener("click", () => {
      window.location.href = "projects/";
    });
  }

  function initContactPopup() {
    const { contactPopup, contactClose } = dom;
